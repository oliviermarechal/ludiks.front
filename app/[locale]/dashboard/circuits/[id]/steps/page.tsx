...
export default function CircuitStepsPage() {
    ...
    const handleManualStepsSubmit = async (data: ManualStepsFormData) => {
        const steps = data.steps.map((step, index) => ({
            name: step.name || `Niveau ${index + 1}`,
            description: step.description || ``,
            eventName: "circuit_progress",
    ...
    }, [mode, generatorValues, fields, manualSteps]);
            ...
            <div className="max-w-5xl mx-auto space-y-12">
                <div className="space-y-4">
                    ...
                    <p className="text-lg text-white/70">
                        Définissez les étapes de progression de votre circuit
                    </p>
                </div>
                ...
                <Tabs.Root value={mode} onValueChange={handleModeChange}>
                    <Tabs.List className="flex rounded-lg overflow-hidden mb-8">
                        <Tabs.Trigger 
                            value="manual" 
                            className={cn(
                                "flex-1 py-3 px-6 text-sm font-medium border transition-all duration-200",
                    ...
                    <Tabs.Content value="manual">
                        <form onSubmit={handleManualSubmit(handleManualStepsSubmit)} className="space-y-8">
                            <div className="space-y-4">
                                {fields.map((field, index) => (
                                    <div key={field.id} className="flex items-center gap-4 p-6 rounded-xl border border-primary/20 bg-black/20 hover:border-primary/40 hover:bg-black/40 transition-all duration-200">
                                        <div className="w-20 shrink-0 font-medium text-white/70">
                                            Étape {index + 1}
                                        </div>
                                        ...
                                        <div className="flex-1">
                                            <Input
                                                ...
                                                placeholder="Nombre d'actions requises"
                            ...
                            {fields.length > 1 && (
                                <div className="space-y-4 p-6 rounded-xl border border-primary/20 bg-black/20">
                                    <Label className="text-lg font-medium text-white">Aperçu de la progression</Label>
                                    <StepPreviewChart steps={previewSteps} />
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        {previewSteps.map((step, index) => (
                                            <div key={index} className="text-sm text-secondary/70">
                                                Étape {index + 1}: {step.completionThreshold} actions
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-end">
                                <Button 
                                    type="submit"
                                    className="bg-secondary hover:bg-secondary/90 text-black font-medium px-8 py-6 text-base"
                                >
                    ...
                    <Tabs.Content value="generator">
                        <form onSubmit={handleGeneratorFormSubmit(handleGeneratorSubmit)} className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    <Label className="text-white">Nombre d&apos;étapes</Label>
                                    <Input
                                        type="number"
                                        min={2}
                                        max={100}
                                ...
                                <div className="space-y-3">
                                    <Label className="text-white">Valeur maximale</Label>
                                    <Input
                                        type="number"
                                        min={1}
                                ...
                                <div className="space-y-3">
                                    <Label className="text-white">Type de progression</Label>
                                    <Select 
                                ...
                                {generatorValues.curve === "power" && (
                                    <div className="space-y-4">
                                        <div className="flex justify-between">
                                            <Label className="text-white">Exposant</Label>
